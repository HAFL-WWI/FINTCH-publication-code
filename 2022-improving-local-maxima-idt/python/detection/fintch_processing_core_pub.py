...
def process_perimeter(record, db_connection):
    ...
    #INSERT INTO fintch.diss_perimeter(geom, slope, slope_class, aspect, aspect_class, z, veg_code, veg_subcode, veg_de, hdom, dg, nh, fst, perimeter_id, source_id, flaeche_id)
	#VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
...
def process_trees(record,db_connection):
    ...
    # df = pd.concat(picked_trees, ignore_index=True)
    # geometry = [Point(xy) for xy in zip(df.x, df.y)]
    # picked_trees_df = gpd.GeoDataFrame(df, crs=crs, geometry=geometry)
